#!/usr/bin/env python3
"""
Project_system 完整驗證腳本（Windows 相容版本）
驗證所有模塊的功能和改進
（跳過 PiCamera 依賴的模塊）
"""
...
def test_logger():
    ...
    try:
        ...
        logger.info("INFO 級別日誌")
        logger.warning("WARNING 級別日誌")
...
def test_ct_calculator():
    ...
    try:
        ...
        print(f"  ✓ 主要方法:")
        print(f"    - calculate(baseline_begin, baseline_end) -> List[float]")
        print(f"    - _load_data() -> bool")
        print(f"    - _normalize(baseline_begin, baseline_end)")
...
# ==================== 測試 6: 合併模塊 ====================
def test_merge():
    print_header("測試 6: 合併模塊 (merge.py)")
    ...
    try:
        ...
        for module_name, description in modules_to_test:
            try:
                ...
                print(f"  ✗ {module_name:20} - {str(e)[:50]}")
...
def test_improvements():
    ...
    print(f"  ✓ 優化方法: cv2.circle() 替代嵌套迴圈")
    ...
    print_section("新增功能")
    print(f"  ✓ CtCalculator 類 (無全局狀態)")
    print(f"  ✓ QPCRServer 類 (命令分派)")
    print(f"  ✓ utils 工具庫 (協議解析)")
    print(f"  ✓ config 配置系統 (80+ 參數)")
    print(f"  ✓ logger 日誌系統 (文件輪轉)")
...
def main():
    ...
    print(f"\n📊 成績: {passed}/{total} 測試通過 ({passed*100//total}%)")
    
    if passed == total:
        print("\n🎉 所有測試都通過了！")
        print("✨ 系統已準備好進行生產部署或樹莓派測試。\n")
    else:
